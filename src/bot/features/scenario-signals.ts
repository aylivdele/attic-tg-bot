...
<i>*Регистрация именно по этой ссылке откроет доступ к скидке на сигналы и другие продукты компании</i>`, { parseMode: 'HTML', keyboard: signalsBootcampKeyboard(scenarioSignalsCallbackData, bootcamp2), leaveLastMessage: true })
})
...
5. После оплаты и ввода UID на сайте, доступ откроется автоматически для каждой выбранной группы

Готово! ✅

💡 <i>После выполнения шага 2 (при условии регистрации по моей ссылке) у тебя также откроются 3 урока в нашей CRYPTO SCHOOL, где ты разберёшься как правильно открывать сделки по сигналам.</i>
