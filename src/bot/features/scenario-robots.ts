...
feature
  .callbackQuery(scenarioRobotsCallbackData, async (ctx) => {
    ...
    return ctx.answerWithMedia(scenarioRobotsCallbackData, `Посмотри видео и узнай как выстроить: 
...
Смотри прямо сейчас. Через 24 часа доступ будет закрыт`, mainRobotsKeyboard())
  })
...
feature
  .callbackQuery(robotsStatisticsCallbackData, async (ctx) => {
    ...
    return ctx.answerWithMedia(robotsStatisticsCallbackData, `Мы предоставляем портфель из 3х алгоритмов:
...
Верифицированная статистика
Алгоритм на бирже BingX`, robotsStatisticsKeyboard(robotsStatisticsCallbackData))
  })

feature.callbackQuery([statisticsRDescription, statisticsAIDescription, statisticsMDescription], async (ctx) => {
  await ctx.answerCallbackQuery()
  return ctx.answerWithMedia(ctx.callbackQuery.data, null, robotsStatisticsKeyboard(ctx.callbackQuery.data))
})
...
https://bingx.com/partner/attic/`, robotsBootcampKeyboard(scenarioRobotsCallbackData, bootcamp1))
})
...
*Регистрация именно по моей ссылке откроет доступ к бесплатным урокам по крипте`, robotsBootcampKeyboard(robotsBootcampCallbackData, bootcamp2))
})
...
(его можно найти в профиле BingX)`, robotsBootcampKeyboard(bootcamp1, bootcamp3))
})
...
Если хочешь - можешь написать мне лично, и я помогу пройти эти шаги и быстрее подключиться к роботу.`, robotsBootcampKeyboard(bootcamp2))
})
